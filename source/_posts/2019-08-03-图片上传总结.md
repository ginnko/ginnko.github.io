...
3. 属性
   ...
   | :------- | :----------------------------------------------------- |
   | accept   | 接受的文件类型                                         |
   | files    | 文件列表（实现中没有使用这个属性而是使用onChange事件） |
   | multiple | 布尔值，是否支持多选                                   |
...
2. 传输的数据格式
   ...
   传输介质使用FormData，以base64或者Blob的格式都可以，后者使用起来
...
### 1. 基本结构
 	...
 	1. 底层处理逻辑（稳定的）
     - 基本结构 :heavy_check_mark:
     - 图片文件​ :heavy_check_mark:
     - 图片上传 :heavy_check_mark:
	2. 上层处理样式、交互及使用场景（可变的）
    - 图片处理 :heavy_check_mark:
      - 压缩
      - 列表展示
      - 属性添加
    - 定义传输过程中交互
...
参考：https://blog.hhking.cn/2018/11/29/html5-img-upload/